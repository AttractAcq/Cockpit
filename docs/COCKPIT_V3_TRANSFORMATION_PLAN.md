...
**Result (2026-08-21/22).** Onboarded a real pilot business, "Build Pod (Pilot)" (`client_id 444f0b63-e937-4d8e-8831-939ea9394740`), sourced from a real founder transcript, through the actual Cockpit UI — not simulated, not SQL-only. Ran it as far as the architecture would honestly let it go, then stopped at a real, well-evidenced blocker rather than push through with fabricated or rubber-stamped content. This step's deliverable — per its own definition above — is the friction catalog, not a completed loop; that catalog is what follows.

*Three real, production-affecting bugs found and fixed* (all deployed to `xivewedajschthjlblfb`, committed to this branch): (1) `ClientDirectory`'s primary "+ New Client" nav entry point used a raw insert that hit a Postgres self-insert-visibility RLS failure (`auth_client_ids()` re-queries `clients` within the same `INSERT` statement) — silently broken for any admin, any client, since before this session, because AA's own client row was seeded via a migration that bypasses RLS. Fixed by routing through the already-working `onboard_client` RPC. (2) Jarvis's dispatcher surfaced only the generic Supabase SDK error string on every failed edge-function call, hiding the callee's real `{ message, code }` body and directly degrading Jarvis's own diagnosis of failures. Fixed across all 5 call sites. (3) `generate-phase-1-file`'s 3000-token budget silently truncated longer Context Files, surfacing as a misleading "invalid JSON" error instead of an honest truncation failure — root cause of most of the Phase 1 generation failures hit this session. Fixed (8000 tokens + `rejectTruncation`), plus added a per-file "Generate" recovery action to `ContextFilesPanel` so a single failed file no longer requires re-running all 21.

*Context OS (Phase 1) closed out clean*: all 21 Context Files generated with real, non-fabricated content and approved after a genuine content-quality review (not a rubber stamp) — confirming the existing Phase 1 architecture generalizes to a second business once its one real bug (above) is fixed.

*Market OS is where this step's real finding is.* Market OS ran to completion twice — once as the original run, once as a full regeneration after "Request changes" — and **both releases had to be rejected**, for a decisive, non-retriable reason: on the second run, every one of the 5 `client_research_steps` rows reported `status: completed`, yet direct inspection of `client_intelligence_records` showed 3 of 5 modules were literal placeholder rows (`title`/`summary` = `"placeholder"`) and a 4th contained real, well-formed prose on the **wrong topic** (word-for-word duplicating a different module's content). The first run corrupted a *different* pair of modules the same way. `status: completed` is therefore not a reliable signal of real content in this provider, the corruption is effectively random across modules and across runs, and nothing in the UI surfaces this — an operator using only the Cockpit UI as designed would see "5/5 Completed" and a release ready to approve. This is a real, systemic bug in `market-research-provider.ts` (a `isPlaceholderText`-style guard exists in the sibling `competitor-research-provider.ts`, from an earlier hardening pass, but was never ported here — and wouldn't fully catch the wrong-topic case either), correctly out of scope to fix live in this session. Because Avatar OS and Competitor OS both hard-require an *approved* Market OS release (`APPROVED_MARKET_OS_REQUIRED`, confirmed live), neither could be exercised this session — a real, enforced sequential gate, not just conceptual ordering. The loop stops here honestly rather than faking a downstream run on top of a release that should never be approved.

*Other real findings, roughly in order of severity*: a 150s Supabase Edge Function "Request idle timeout" is real, hard, and plan-independent (confirmed by a disposable probe function, `IDLE_TIMEOUT` at 150.6s) — the existing 135–145s self-abort margins across the 5 intelligence providers are correctly tuned, not stale; but the outer `jarvis-turn` dispatcher itself has no equivalent margin and can hit the same wall, and a turn killed mid-tool-call leaves a dangling `tool_use` with no matching `tool_result` in the stored chat history — which permanently 400s every future turn in that thread (the same failure class already fixed once, inside the intelligence providers' own loops, per this repo's Competitor OS hardening notes — never ported to the Jarvis dispatcher's own history). Recovery is a new chat thread; the underlying run state survives in the DB. Separately, the chat UI can silently fall multiple minutes behind rendering a long chained turn with no visible sign anything is wrong (indistinguishable, from the UI alone, from a genuinely dead request) — confirmed by direct DB query while the UI still showed a stale `calling…` frame. Full findings, root causes, and exact reproduction steps are in the session's working catalog (not committed — summarized here); the two structural fixes worth carrying forward are the placeholder/wrong-topic guard in the intelligence providers and dangling-`tool_use` protection in the Jarvis dispatcher itself.

*Exit gate: not met, honestly.* Business #2's loop did not complete intelligence → strategy → offer → marketing → sales → delivery → finance — it stopped at Market OS by design, once continuing would have meant either approving fabricated-looking content or burning further sessions on a bug this step correctly identified as out of scope to fix live. That stop is itself the exit gate's real signal: the architecture generalizes cleanly everywhere it was reached (client onboarding once fixed, Context OS, the Jarvis tool/gate pattern, the business_id/client_id bridge — see below) and the one place it doesn't is a concrete, well-evidenced provider bug, not an architecture problem requiring the `business_id` migration this plan deferred in §3.

*`business_id`/`client_id` bridge finding*: confirmed in code, not just plan-doc prose — `list_sales_pipeline`'s own tool description states the bridge directly, and `run-market-os`'s `loadMarketAuthority` hard-requires `clients.id`. Only `sales_leads` (and, per this session's code read, `team_channels`/`team_messages`, `sales_proposals` via the lead) are genuinely `business_id`-native; every other department — Intelligence, Ideation, Offers, Finance, Knowledge, Marketing, Master AI — is still 100% `client_id`-gated, unconditionally, not just as a fallback. This confirms §3's claim precisely and gives it a real citation instead of a guess.
