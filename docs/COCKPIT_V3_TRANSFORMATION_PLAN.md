...
**Not yet done:** extending `jarvis-turn`'s tool surface to actually reason across departments (step 3 — it currently only covers Marketing's sub-systems) hasn't been started; nothing has been live-tested against a real cross-department question yet. Verified: typecheck, lint (0 errors, same 6 pre-existing warnings), build, `check:edge-functions` (unaffected — 0 new edge functions), full suite 1102/1102 (3 new), `git diff --check` clean.
