...
**Not yet done, honestly:** the sidebar still shows the current 7-item nav, not the target 10-group Business OS shape (that's Step 2's job); the ~20+ panels inside `ClientDetailPage` still resolve `clientId` via prop-drilling from its own `useParams`, unmigrated (the single largest remaining piece of this step — deliberately not attempted in one pass given its size and the zero-regression bar); Finance, Delivery, Team, Comms, Knowledge pages haven't been touched. The exit gate is not being claimed met.
