...
**Not yet done, honestly: Marketing** — the one system in the bridge table big enough that it can't be a single-component extraction like the four rehomes above (Automations, Team, Knowledge, Finance). Marketing spans roughly eight already-real panels (Offers, Ideation, Content Creation, Distribution, Iteration, Campaigns, and more) currently reached only as sections inside `ClientDetailPage`'s own per-client tab switcher — rehoming it as a genuine top-level Marketing OS means a new top-level page with its own client picker (the now-proven `client_id`/`BusinessContext` bridge pattern) plus its own second-level navigation across all eight, not a one-file extraction. Scoped as its own dedicated pass, not attempted in this one. Opportunity OS/Activity Log's fold into Overview/Master AI is deferred until those pages exist in more than kernel form (Step 4/5 territory in practice, even though listed here). The exit gate is not being claimed met.
