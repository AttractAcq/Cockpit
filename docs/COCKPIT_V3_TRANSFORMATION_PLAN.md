...
**Not yet done, honestly:** the exit gate's own headline claim — a real Instagram conversation actually flowing through and attributing correctly — remains exactly where Phase 10 left it: blocked on real Meta secrets (`AA_META_VERIFY_TOKEN`, `META_APP_SECRET`) and Alex's own live test, deliberately not exercised by this session per his explicit choice in Phase 10. No amount of UI work in this step closes that; it's an external dependency, not an engineering one. Team-as-workspace (channels/threads/mentions — the one genuinely greenfield piece of this step) and Documents (the folder repository over the provenance layer) haven't been started.
