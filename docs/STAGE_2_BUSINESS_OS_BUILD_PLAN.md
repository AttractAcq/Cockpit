...
**Status:** Phase 00 complete (2026-08-20). Phase 01 shipped and live-tested (2026-08-20); its exit gate is a genuine two-week real-usage wait, earliest completion 2026-09-03 — see Phase 01's own card below. Phase 02 complete (2026-08-20), started and finished ahead of Phase 01's exit gate on Alex's explicit instruction — safe to do because Phase 02 is genuinely additive and provably isolated (zero inbound foreign keys into `businesses` from anywhere else in the schema), so it carries no risk to Phase 01's still-running usage-gate clock. Phase 03 complete (2026-08-20) — surfaced a real, unreviewed live subsystem (see Phase 03's own card: the unmerged Facebook branch stack, deployed to production, invisible until now). Phase 04 complete (2026-08-20) — the thinnest phase yet, zero new schema. Phase 05 complete (2026-08-20) — full dependency trace found no landmines (nothing was dropped), a real naming collision caught before it happened, and a real UI-ordering regression caught by the test suite and fixed before merge. Phase 06 shipped and live-tested (2026-08-20) — Decision 2's literal text ("reuse `client_work_items`") turned out to be infeasible once checked against the real constraint (`client_work_items.client_id` is `not null`, but a lead is by definition pre-client); built bespoke, cross-business `sales_leads`/`sales_conversations` tables instead, preserving Decision 2's actual intent (thin CRM, not an integrated tool) — see Phase 06's own card. Its exit gate is a genuine real-usage wait, same class as Phase 01's. Phase 07 shipped and live-tested (2026-08-20) — reconciled against Phase 00's own Operations-page audit before writing anything: Clients, Onboarding, and Tasks (`client_work_items`) were all already real, so the only genuine gap was Projects (the grouping missing above individual work items) and Deliverables (a client-facing output with its own review/approval state); built exactly those two tables, plus an additive `project_id` column reusing `client_work_items` for Tasks unchanged, within Decision 4's ≤3-table budget. SOPs/Quality/Reporting from the phase card's longer wishlist are deliberately deferred — see Phase 07's own card. Its exit gate is also a genuine real-usage wait, same class as Phase 01's and Phase 06's. Phase 08 shipped and live-tested (2026-08-20) — extends Stage O's already-real `client_cost_ledger`/`client_margin_summary` rather than building cost/margin schema from scratch: one new table (`client_finance_periods`, a real accounting period with a reconciliation step that snapshots total_cost/margin permanently, distinct from the mutable `monthly_revenue_estimate`) plus a bulk, atomic CSV-import RPC. Accounting-platform/payment-processor/bank-feed integration is explicitly deferred until CSV import earns trust through real usage, per the phase's own step-gating. Its exit gate is also a genuine real-usage wait, same class as Phase 01's, 06's, and 07's. Phase 09 complete (2026-08-20) — the thinnest phase yet (zero new schema, zero new RPCs, zero new edge functions), and unlike every other Stage 2 exit gate so far, Team's own exit gate ("every current human and agent role accurately represented, no fictional entries") is a correctness check verifiable in-session, not a real-usage wait — its own step 4 (cross-check against actual staffing) found and removed a real orphaned test-fixture auth user from an earlier, unrelated body of work, with explicit sign-off obtained before deleting. Phase 10 shipped (2026-08-20) — the first phase requiring a real external integration, not just Supabase schema: found this codebase already has a real, live Meta Graph API integration for Distribution's Instagram publishing (`_shared/instagram-publish.ts`), and built `comms_identities`/`comms_messages` plus `meta-instagram-webhook` (inbound, HMAC-signature-verified, JWT disabled by design) and `send-instagram-message` (outbound) to reuse its exact credential-resolution pattern rather than inventing a new one. Identity resolution is deliberately manual (Instagram gives no email/phone to auto-match against). The real Meta webhook/send calls were explicitly not live-tested this session, by Alex's own choice — pending real secrets (`AA_META_VERIFY_TOKEN`, `META_APP_SECRET`) and his own real-world test. Its exit gate is a genuine real-usage wait, same class as Phase 01's, 06's, 07's, and 08's — and additionally gated on those secrets being set and a first real conversation actually flowing through. Phase 11 shipped (2026-08-20), built and run ahead of its own stated prerequisite ("Finance (08) and Sales (06) each have multiple real reconciled cycles behind them — don't start early") on Alex's explicit override, obtained by asking plainly before writing any code — see Phase 11's own card. The Detect/Score/Explain logic itself is real and deterministic, not fabricated, but findings against real AA data are genuinely sparse today because the Finance/Sales history it reads is genuinely thin — that is honest behaviour, not a bug. A real correctness bug (a correlated-subquery column-shadowing bug that silently broke idempotency) was found via live-testing and fixed before merge — see Phase 11's own card. Its exit gate (a full quarter minimum of confirmed-trustworthy human review) is explicitly NOT being claimed met.
...
| Finance | `client_cost_ledger` + `client_margin_summary` view + `clients.monthly_revenue_estimate` (Stage O) **plus** `client_finance_periods` + bulk CSV import (Phase 08, shipped, exit gate pending) | exists (CSV import + reconciliation), integrations deferred | Not greenfield. `client_finance_periods` adds the one genuine gap: a real accounting period with a reconciliation step, distinct from the mutable, historyless `monthly_revenue_estimate`. Accounting-platform/payment-processor/bank-feed integration explicitly deferred until CSV import earns trust through real usage, per the phase's own step-gating. |
| Team | `team_members` assignment (all 9 Stage O roles, real client-scoped read visibility) + `users` role vocabulary **plus** a Team Directory (Phase 09, complete) unifying every human role with every current live automation from Phase 03's registry | exists | Stage O's real fix: `auth_client_ids()` now correctly routes every staff role through `team_members`, not just admin/account_manager. Zero new schema this phase — the directory is entirely composed from already-real data. Its own step-4 cross-check found and removed a real orphaned test-fixture auth user. Fine-grained per-role *write* permissions and capacity modeling remain out of scope; capacity is stated as honestly untracked, not fabricated. |
...
### Phase 08 — Finance — **shipped and live-tested (2026-08-20), exit gate pending**

**Goal:** built to the Blueprint's own phased sequencing almost exactly — it's the best-sequenced section of that document.

- **Builds on:** nothing existing — greenfield; the sequencing itself is the discipline. **Corrected during implementation:** not greenfield — Phase 00's own audit table already said so ("Finance moves from net-new to partial"), the phase card's "Builds on: nothing existing" line just never got reconciled against that finding before now. `client_cost_ledger`/`client_margin_summary`/`clients.monthly_revenue_estimate` are real, live Stage O schema; this phase extends them.
- **Deferred:** feeding Finance data to any agent decision, until numbers are reconciled and trusted for at least one full period. Also deferred, per the phase's own step-gating: accounting-platform integration, payment processors, direct bank feeds, autonomous reconciliation — all sequenced *after* CSV import earns trust through real usage, which hasn't happened yet.
...
**Exit gate:** real AA numbers, reconciled, for one full accounting period, before Finance data touches any agent. **Genuinely not shortcuttable by disposable test fixtures**, same class of gate as Phase 01's, 06's, and 07's — this phase can be shipped, live-tested, and merged, but is not "complete" until Alex runs one full real accounting period through it.

**Result — the real gap was a period concept, not new cost/margin tables.** `client_cost_ledger` (transaction-level costs) and `client_margin_summary` (a live-computed rollup) were already real and live. But `clients.monthly_revenue_estimate` is a single, mutable, historyless scalar — it can be overwritten at any time with no record of what it was last period, and nothing marks a period "done." That makes "reconciled... for one full accounting period" impossible to express in the existing schema, not merely inconvenient. Built exactly one new table, `client_finance_periods`: a real accounting period (`period_start`/`period_end`) that gets opened, then reconciled once with a real `actual_revenue` figure — at which point `total_cost` is computed from `client_cost_ledger` for that exact date range and stored as a permanent snapshot alongside the resulting `margin`, both enforced present-together by a table CHECK constraint. Deliberately a snapshot, not a live view: a cost entry backdated into an already-reconciled period must not silently change that period's already-reported numbers — confirmed by a live test that imported a cost entry after reconciliation and verified the stored `total_cost` didn't move. A period can only be reconciled once (re-reconciling raises `CONFLICT`), and `open_finance_period` rejects any date range overlapping an existing period for that client.

CSV import is `import_cost_entries`, a bulk, atomic counterpart to Stage O's own `record_cost_entry` RPC — one JSON array of rows in, all-or-nothing (a single invalid category or negative amount rolls back the entire batch, confirmed live: a 2-good-1-bad batch left zero new rows, not two). Parsing itself is a small, dependency-free pure function (`finance-csv.ts`, `cost_category,amount,occurred_at[,notes]`, optional header row) with its own real behavioural test suite run directly against the actual parser — not just source-text assertions — covering the happy path, blank-line handling, and every rejection case (unknown category, negative/non-numeric amount, unparseable date, malformed row), each with a 1-indexed line number so a bad CSV file tells the operator exactly where to look.

Both new RPCs (plus `open_finance_period`) reuse `record_cost_entry`'s own narrower `admin`/`account_manager`-only auth gate, not the broader 7-role staff list every other Stage 2 RPC in this plan uses — money mutations get the tighter existing precedent, not a new one invented for this phase.

No new UI section: the CSV-import control and the Finance-periods list both live inside the existing "Cost & Margin" sub-tab in Operations Control, rather than becoming a 9th/10th top-level sub-tab of their own — 0 new UI sections, well within Decision 4's budget (the tab itself was real before this phase).

**Verification:** live-tested all three RPCs against `xivewedajschthjlblfb` inside a rolled-back transaction — period opened, overlap rejected, a 3-row batch imported (one deliberately outside the period's date range), a 2-good-1-bad batch confirmed fully rolled back (atomicity), the period reconciled with `total_cost` correctly excluding the out-of-range row, double-reconciliation rejected, and a backdated post-reconciliation import confirmed *not* to move the already-stored snapshot. Separate unauthenticated/nonexistent-user auth-rejection pass. `get_advisors`: zero new table-level findings; the three new RPCs carry only the same already-accepted "authenticated can call this SECURITY DEFINER RPC" class every other staff-gated RPC in this schema carries. Full suite: typecheck, lint (0 errors, same 6 pre-existing warnings), `check:edge-functions` (unchanged — 0 new edge functions), 989/989 tests (15 new, including real behavioural tests of the CSV parser, not just source-text assertions), build, `git diff --check` all clean.

### Phase 09 — Team — **complete (2026-08-20)**
...
- **Deferred:** capacity-based auto-allocation of work. That's Executive AI territory (12). Also deferred: a dedicated Roles/Responsibilities/Capacity schema — see Result below.
...
**Exit gate:** every current human and agent role at AA is accurately represented — no fictional org chart entries. **Unlike every other Stage 2 phase's exit gate so far, this one isn't a real-usage calendar wait — it's a correctness check, verifiable in-session against live data, not something only Alex can close out later.**

**Result — the thinnest phase yet, and its own step 4 caught a real problem.** `team_members` (client-scoped assignment), `users` (the real role vocabulary), and Phase 03's governed edge-function registry were all already real per Phase 00's audit — there was nothing genuinely missing to justify new schema. Built zero new tables, zero new RPCs, zero new edge functions: the Team Directory is a read-only view inside the existing "Team & Roles" sub-tab, composed entirely from `fetchStaffUsers()`/`fetchTeamMembers()` (humans) and `fetchWorkflows()` filtered to non-retired (agents, 90 of 103) — nothing invented. "Responsibilities" for a human is its real role label; for an agent, its real registry `purpose` text. Capacity is stated honestly as not tracked yet (Phase 00's own audit already found capacity/performance data isn't real) rather than fabricated, matching the disclosure precedent Phase 05's Campaigns panel set for its own deferred fields.

**Step 4 — cross-check against actual current staffing for fictional entries — is not a formality; it found one.** Querying the real `users` table before considering the directory done turned up exactly 2 rows: Alex Thomas (real, admin) and `avatar-os-test-1786717849@example.invalid` (role: admin) — an orphaned disposable test-fixture auth user from earlier, unrelated Avatar OS work that was never cleaned up. Left in place, it would have shown up in the new directory as a fictional "Human" admin — precisely the failure mode this exit gate exists to catch. Confirmed zero foreign-key references anywhere in the schema (`team_members`, `activity_log`, `client_work_items` all clean) before deleting; flagged to Alex and deleted only after explicit sign-off, per this session's standing discipline around destructive actions on live auth data. `auth.users`/`public.users` now both show exactly 1 row. The directory's own step 4 is what surfaced this — not a coincidence, the process working as designed.

**Verification:** no migration, so no `get_advisors` re-run needed. Full suite: typecheck, lint (0 errors, same 6 pre-existing warnings), `check:edge-functions` (unchanged — 0 new edge functions, confirms 90 non-retired/13 retired), 995/995 tests (6 new), build, `git diff --check` all clean. Live-verified post-cleanup: `auth.users` count = 1, `public.users` count = 1, matching.

**Exit gate met.** Every human role shown (1, real) and every agent role shown (90, real and current) trace to live data with zero fictional entries — confirmed, not assumed.

### Phase 10 — Communications Hub — **shipped (2026-08-20), exit gate pending**
...
- **Builds on:** Sales (06) and Delivery (07) as its first consumers. Also builds on Distribution's real, existing Meta Graph API integration — see Result below.
- **Deferred:** full channel breadth. Start with one or two channels, not the whole list. Also deferred: automatic identity matching (see Result).
...
**Exit gate:** one channel's real conversations flow through it and attribute correctly to the right Sales or Delivery record. **Genuinely not shortcuttable by disposable test fixtures**, same class of gate as Phases 01/06/07/08 — additionally gated on real Meta secrets being set and Alex's own live test, since this phase's real external calls were deliberately not exercised this session (see Result).

**Result — the first Stage 2 phase requiring a real external integration, not just Supabase schema, and a major discovery changed the credential plan before any code was written.** Alex confirmed Meta Business verification was complete with real credentials available. Before asking for a fresh set of secrets, checked what already existed: this codebase already has a real, live, working Meta Graph API integration for Distribution's organic Instagram publishing (`_shared/instagram-publish.ts`, `resolveMetaConfig`) — a real access token resolved via `_GLOBAL_META_SYSTEM_USER_TOKEN`/`META_SYSTEM_USER_TOKEN`/per-account Vault, calling `graph.facebook.com`. Reusing that exact credential-resolution priority order (new `_shared/comms-meta.ts`, deliberately separate from `instagram-publish.ts` itself — sending a DM reply and publishing a feed post are different Graph API capabilities) meant only two genuinely new secrets were needed instead of five: `AA_META_VERIFY_TOKEN` (reusing the exact name the retired legacy `meta-webhook` function already used) and `META_APP_SECRET` (for HMAC webhook-signature verification, which the one-way publish pipeline never needed). Confirmed and agreed with Alex before writing any code.

Two new tables: `comms_identities` (a real external-platform identity, v1 Instagram-only, optionally linked to a `sales_leads` or `clients` row) and `comms_messages` (the conversation timeline, idempotent on `external_message_id` so a Meta webhook retry can't duplicate a row — confirmed live). Identity resolution is deliberately manual, not automatic fuzzy-matching: Instagram gives no email/phone to match against, and building an AI auto-linker ahead of any real usage would repeat the exact premature-automation mistake Principle 01 and the `command_center_notes` precedent both already warn against. A human links a conversation to a lead or client; that's what "attribute correctly" means for v1.

Two new edge functions — genuinely required here, unlike every prior Stage 2 phase's "0 new edge functions" (Decision 4's default only applies "where a direct RPC suffices"; a public webhook receiving unauthenticated Meta traffic and an outbound Graph API call both structurally cannot be RPCs). `meta-instagram-webhook`: `verify_jwt=false` by design (Meta sends no Supabase JWT), authenticated instead via `hub.verify_token` on the GET handshake and HMAC `X-Hub-Signature-256` verification on every POST — a new `webhook_meta_verified` registry profile, since no existing profile's caller/auth/lifecycle contract matched (per the registry maintenance doc's own rule: reuse only if every inherited field is correct). Always answers Meta with HTTP 200 once the signature is valid, even on a downstream processing error — the exact operational lesson the retired legacy `meta-webhook` function's own comments already documented ("non-200 causes retry storms"), carried forward into genuinely new code rather than reviving dead code that referenced the retired `entities`/`conversations` schema. `send-instagram-message`: staff-JWT-gated (a new `ui_cross_business` profile — the existing `ui_authority`/`ui_operational` profiles both hard-code `clientScope: required`, which doesn't fit a cross-business inbox), records the outbound message only after Meta confirms success, never speculatively before.

Both edge function deploys were held for Alex's explicit sign-off (Phase 00's own human-approval-gate list) before `deploy_edge_function` was called, given as a single combined decision after the code was written and reviewed rather than blocking mid-build. Deployed and `ACTIVE`. No live call against the real Meta API was made this session, by Alex's own explicit choice — real credentials exist but testing the actual webhook/send path is his to do.

**Verification:** live-tested `record_comms_message` and `link_comms_identity` against `xivewedajschthjlblfb` inside rolled-back transactions as `service_role` and staff respectively — identity find-or-create and reuse across repeated messages from the same sender, confirmed webhook-replay idempotency (the exact same `external_message_id` submitted twice returns the same message id and does not create a duplicate row), outbound `sent_by` attribution, unsupported-platform rejection, staff-only linking with dual-link (lead + client simultaneously) rejection, unlink, nonexistent-lead rejection, and confirmed an `authenticated` (non-service-role) caller cannot call `record_comms_message` directly. Separate anon/nonexistent-user auth-rejection pass. `get_advisors`: zero new table-level findings; the one staff-callable RPC carries only the same already-accepted advisory class every other staff-gated RPC in this schema carries; `record_comms_message` (service-role-only) triggers no advisory at all, since it isn't callable by signed-in users. Full suite: typecheck, lint (0 errors, same 6 pre-existing warnings), `check:edge-functions` (105 functions, 82 UI-called, 23 non-UI, 13 retired — 2 new, both registered with narrowly-scoped new profiles, hardcoded counts in `tests/edge-function-registry.test.ts` updated in the same commit), 1013/1013 tests (18 new), build, `git diff --check` all clean.

### Phase 11 — Opportunity OS — **shipped (2026-08-20), built ahead of its own prerequisite on explicit override**
...
**Result — step 1's own check failed, and the phase was built anyway, on explicit instruction.** Checked before writing anything: as of this migration, neither Finance (08) nor Sales (06) has cleared even its own *single*-cycle exit gate yet, let alone "multiple real reconciled cycles." This was surfaced plainly to Alex rather than silently built around or silently skipped; his explicit answer was "Override the gate and build/run it now anyway." Given that override, the build proceeded under one firm honesty commitment: the detection logic itself had to be real and deterministic — never a fabricated "AI insight" and never seeded/fake findings — even though run today it will necessarily find little or nothing, because the Finance/Sales history it reads over is genuinely still sparse. That sparsity is correct, expected behaviour, not something this phase's card papers over.

One new table (`opportunity_os_findings`) and two staff-gated RPCs (`run_opportunity_detection`, `review_opportunity_finding`), zero new edge functions — Detect/Score/Explain here is deterministic SQL over three already-real tables (`client_finance_periods`, `sales_leads`/`sales_conversations`, `client_performance_scores`), not an external AI call, so a plpgsql RPC is the right shape. Three finding types, each a plain auditable rule: `margin_risk` (a reconciled finance period closing under 10% margin), `stalled_lead` (an open sales lead with no logged conversation in 14+ days), `underperforming_channel` (a performance score in the bottom quartile of that client's own recent scores). Every finding's `source_refs` cites the exact source row it came from — never a black box. Named `opportunity_os_findings`, not the bare "opportunities" the original request would have suggested — `content_opportunities`/`content_opportunity_scores`/`content_opportunity_sources` and `ad_opportunities` already use that word for a different concept (a piece of content/ad worth producing), a repeat of the exact naming collision Phase 05's dependency trace caught for Campaigns.

**A real bug, found via live-testing before any commit:** the idempotency guard inside `run_opportunity_detection` used a correlated `not exists (select 1 from opportunity_os_findings f where ...)` subquery with a bare, unqualified `id` in one branch — since `opportunity_os_findings` itself has an `id` column, Postgres silently resolved it to the subquery's own row instead of the intended outer source row, breaking idempotency (a second run produced a duplicate finding instead of skipping the already-reported one). Fixed by fully alias-qualifying every reference (`cfp.id`, `l.id`, `ps.id`) across all three detection branches before the migration was ever committed, with the failure mode documented directly in the migration's own comments as a discipline lesson for future correlated subqueries in this codebase.

Frontend: a new top-level "Opportunities" nav page (`OpportunitiesPage.tsx`, `src/lib/opportunity.ts`, `src/types/opportunity.ts`) — a single cross-client report, not nested inside a client detail page, matching the phase's own "specialist reader" framing. Staff pick a client and click "Run detection"; findings list with client/status/type filters, each showing its explanation and citing its source; a `pending_review` finding gets "Confirm useful" / "Dismiss" with optional notes, which is the only thing that closes a finding out — nothing here auto-acts.

**Live-tested end-to-end** against `xivewedajschthjlblfb` inside rolled-back transactions with disposable `ZZ-TEST`-prefixed fixtures (a thin-margin reconciled finance period, a stalled sales lead, service-role direct writes re-queried by unique marker strings from the `authenticated`-role block, since both source tables are RPC-write-only): happy-path detection produced findings citing the real fixture rows; re-running detection with the fix applied correctly produced exactly 0 new findings (idempotency confirmed — this was the run that caught the bug above before the fix); reviewing a finding as `confirmed_useful` correctly set `reviewed_by`/`reviewed_at`; an invalid review status was correctly rejected; a nonexistent client was correctly rejected; and the standard anon/non-staff auth-rejection pass was clean on both RPCs. `get_advisors` showed zero table-level findings and only the same already-accepted "SECURITY DEFINER callable by authenticated" class every other Stage 2 admin RPC carries.

**Exit gate — explicitly NOT being claimed met.** The phase's own exit gate ("a full quarter minimum" review period confirming the scoring/explanations are actually trustworthy) cannot be satisfied by a same-session live test with disposable fixtures, and this card does not pretend otherwise. What's shipped is the real, working detection/review mechanism; whether its scoring and explanations hold up is a question only real-usage time can answer, exactly as the phase's own step 3–4 describe.
