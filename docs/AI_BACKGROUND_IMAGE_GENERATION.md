...
The Edge Function allowlists model-specific size and quality values before claiming a generation. Missing or unsupported models and invalid defaults or caller values fail closed before any OpenAI request. There is no silent fallback from `gpt-image-2` to `gpt-image-1`; the latter is only an explicitly configured compatibility profile.
