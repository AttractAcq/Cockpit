...
assert.equal(insights.isTerminallyExpiredStory(story, false), false, "expired Story without a prior terminal skip remains eligible once");
assert.equal(insights.isTerminallyExpiredStory(story, true), true, "expired Story with a prior skipped_expired attempt is excluded");
assert.equal(insights.isTerminallyExpiredStory(feed, true), false, "feed failures or skip-like history must not become terminal");
assert.equal(insights.nextDueSnapshot(feed, ["t_plus_1h"], new Date("2026-07-01T01:00:00Z")), null, "an existing API label is excluded");
assert.equal(insights.nextDueSnapshot(story, [], new Date("2026-07-01T06:00:00Z")).label, "story_t_plus_1h", "a due non-expired Story remains eligible");
...
assert.match(worker, /\.eq\("status", "skipped"\)\.eq\("reason", "skipped_expired"\)/);
assert.match(worker, /isTerminallyExpiredStory\(record, terminallyExpiredStoryIds\.has\(record\.id\)\)/);
