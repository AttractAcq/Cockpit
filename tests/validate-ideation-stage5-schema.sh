#!/usr/bin/env bash
# Exact-schema validation for the Ideation Stage 4 and Stage 5 migrations.
...
container="${IDEATION_TEST_DB_CONTAINER:-aa-ideation-stage5-schema-$RANDOM-$$}"
workspace="$(mktemp -d)"
...
# Stage 4 is already present in the authoritative baseline; Stage 5 is the only
# forward migration under test.
"${psql_cmd[@]}" < "$repo_root/supabase/migrations/20260731000041_ideation_stage5_authority_race.sql"
...
SQL
  ...
  "${psql_cmd[@]}" < "$repo_root/supabase/migrations/20260731000041_ideation_stage5_authority_race.sql" >/dev/null
...
# Stage 5: the authority race needs its own uncommitted lineage.
reset_database
IDEATION_TEST_DB_CONTAINER="$container" IDEATION_TEST_DB_NAME="$database" \
  IDEATION_TEST_DB_USER="$database_user" \
  bash "$repo_root/tests/ideation-stage5-authority-race.sh"

echo "Authoritative Stage 4 and Stage 5 schema, migration, SQL, RLS, adversarial, and concurrency validation passed."
