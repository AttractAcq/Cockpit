#!/usr/bin/env bash
# Exact-schema validation for the Ideation Stage 3 migration.
...
container="${IDEATION_TEST_DB_CONTAINER:-aa-ideation-stage3-schema-$RANDOM-$$}"
workspace="$(mktemp -d)"
...
# Frozen-object snapshot before and after: Stage 3 must not drift Phase 1,
# Phase 2, Stage 1, Stage 2, the Calendar, the masters, or playbooks.
"${psql_cmd[@]}" < "$repo_root/tests/ideation-pr1-schema-snapshot.sql" > "$workspace/before.json"
"${psql_cmd[@]}" < "$repo_root/tests/ideation-stage3-frozen-snapshot.sql" > "$workspace/frozen-before.json"
"${psql_cmd[@]}" < "$repo_root/supabase/migrations/20260729000039_ideation_stage3_calendar_proposal.sql"
"${psql_cmd[@]}" < "$repo_root/tests/ideation-stage3-schema-postflight.sql"
"${psql_cmd[@]}" < "$repo_root/tests/ideation-pr1-schema-snapshot.sql" > "$workspace/after.json"
"${psql_cmd[@]}" < "$repo_root/tests/ideation-stage3-frozen-snapshot.sql" > "$workspace/frozen-after.json"
...
IDEATION_TEST_DB_CONTAINER="$container" IDEATION_TEST_DB_NAME="$database" \
  ...
  bash "$repo_root/tests/ideation-stage3-concurrency.sh"

echo "Authoritative Stage 3 schema, migration, SQL, RLS, adversarial, and concurrency validation passed."
