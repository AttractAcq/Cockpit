...
interface ClientDirectoryProps {
  ...
  title: string;
  /** Builds the route a table row navigates to when clicked. */
            ...
            <table className="w-full text-xs">
              ...
              <tbody>
                {filtered.map((c) => (
                  <tr
                    key={c.id}
                    onClick={() => navigate(getRowPath(c.id))}
                  ...
                  >
                    ...
                    <td className="px-3 py-2.5">
                      {showCalendarShortcut ? (
                        <button className="rounded hover:bg-teal/10 focus:outline-none focus:ring-1 focus:ring-teal/50" onClick={(event) => { event.stopPropagation(); navigate(ROUTES.clientSection(c.id, "calendar")); }} title="Open Phase 3 calendar"><StagePill status={stage3Statuses[c.id] ?? "not_started"} /></button>
                      ) : (
                        <StagePill status={stage3Statuses[c.id] ?? "not_started"} />
                      )}
                    </td>
                    <td className="px-3 py-2.5">
