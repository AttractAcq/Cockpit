...
export function CompetitorOSPanel({ clientId, clientName }: { clientId: string; clientName: string }) {
  ...
  const [progress, setProgress] = useState<RunCompetitorOSStepResponse["progress"] | null>(null);
  ...
  const reload = useCallback(async () => {
    setLoading(true);
    try {
      setWorkspace(await fetchIntelligenceWorkspace(clientId, "competitor_os"));
      setError(null);
    } catch (caught) {
  ...
  async function review(decision: "approved" | "changes_requested") {
    ...
    if (decision === "approved") {
      ...
      const entered = window.prompt("What must change before this Competitor OS can be approved?");
    ...
    try {
      ...
      setMessage(decision === "approved" ? "Competitor OS approved and activated." : "Changes requested. The release is back in draft.");
      await reload();
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : String(caught));
    } finally {
  ...
  const subjectCount = new Set(records.map((record) => recordPayloadValue(record, "subject_key")).filter(Boolean)).size;
  ...
  const buildLabel = working
    ...
    : latest?.status === "approved" || active ? "Refresh Competitor OS" : run?.retryable ? "Resume Competitor OS" : run?.status === "failed" ? "Rebuild Competitor OS" : run ? "Resume Competitor OS" : "Build Competitor OS";
      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-4">
        <div className="flex flex-wrap items-start justify-between gap-4 rounded-[10px] border border-line bg-ink-200 p-4">
          <div className="max-w-2xl">
            ...
            <h2 className="text-base font-medium text-paper">Competitor OS</h2>
            <p className="mt-1 text-xs leading-5 text-paper-3">
              The buyer-visible competitive system: commercial alternatives, substitutes, in-house and do-nothing choices, positioning maps, proof, messaging and distribution observations. Research creates a reviewable draft; only human approval makes it authoritative.
            </p>
          </div>
        ...
        {progress && working && (
          <div className="rounded-[10px] border border-teal/20 bg-teal/5 px-4 py-3 text-xs text-paper">
            Competitor research: {progress.completed} completed, {progress.failed} failed, {progress.total} total modules.
          </div>
        )}
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-6">
          ...
          <Card>
            <span className="text-2xs uppercase text-paper-3">Landscape</span>
            <span className="text-sm text-paper">{subjectCount} subjects</span>
            <span className="text-2xs text-paper-3">{changedCount} changed observations</span>
          </Card>
          <Card>
        ...
        {recordGroups.length > 0 ? (
          ...
          <Panel key={recordType} title={readable(recordType)} meta={`${group.length} records`}>
            <div className="grid gap-px bg-line md:grid-cols-2">
              {group.map((record) => {
                const alternativeType = recordPayloadValue(record, "alternative_type");
                ...
                <div key={record.id} className="bg-ink-200 p-4">
                  <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {alternativeType && <Tag kind="muted">{readable(alternativeType)}</Tag>}
          ...
          <div className="rounded-[10px] border border-dashed border-line bg-ink-200 p-8 text-center">
            <h3 className="text-sm font-medium text-paper">No Competitor OS release yet</h3>
            <p className="mx-auto mt-2 max-w-md text-xs leading-5 text-paper-3">
              Approve Market OS and Avatar OS first, then build Competitor OS to map the choices buyers actually see—without opaque scoring, prescriptions, or competitor copying.
            </p>
          </div>
        )}
