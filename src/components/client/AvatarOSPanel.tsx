...
export function AvatarOSPanel({ clientId, clientName }: { clientId: string; clientName: string }) {
  const [workspace, setWorkspace] = useState<IntelligenceWorkspace | null>(null);
  const [loading, setLoading] = useState(true);
  const [working, setWorking] = useState(false);
  const [progress, setProgress] = useState<RunAvatarOSStepResponse["progress"] | null>(null);
  ...
  const reload = useCallback(async () => {
    setLoading(true);
    try {
      setWorkspace(await fetchIntelligenceWorkspace(clientId, "avatar_os"));
      setError(null);
    } catch (caught) {
  ...
  async function build() {
    ...
    setProgress(null);
    try {
      const result = await driveAvatarOSBuild(clientId, setProgress);
  ...
  async function review(decision: "approved" | "changes_requested") {
    ...
    if (decision === "approved") {
      ...
      const entered = window.prompt("What must change before this Avatar OS can be approved?");
    ...
    try {
      ...
      setMessage(decision === "approved" ? "Avatar OS approved and activated." : "Changes requested. The release is back in draft.");
      await reload();
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : String(caught));
    } finally {
  ...
  if (loading && !workspace) {
    return <div className="flex flex-1 items-center justify-center text-xs text-paper-3">Loading Avatar OS…</div>;
  }
  ...
  const buildLabel = working
    ...
    : latest?.status === "approved" || active ? "Refresh Avatar OS" : run?.retryable ? "Resume Avatar OS" : run?.status === "failed" ? "Rebuild Avatar OS" : run ? "Resume Avatar OS" : "Build Avatar OS";
      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-4">
        <div className="flex flex-wrap items-start justify-between gap-4 rounded-[10px] border border-line bg-ink-200 p-4">
          <div className="max-w-2xl">
            ...
            <h2 className="text-base font-medium text-paper">Avatar OS</h2>
            <p className="mt-1 text-xs leading-5 text-paper-3">
              Evidence-backed buying roles, decision dynamics, triggers, objections, and trust requirements. Research creates a reviewable draft; only human approval makes it authoritative.
            </p>
          </div>
        ...
        {progress && working && (
          <div className="rounded-[10px] border border-teal/20 bg-teal/5 px-4 py-3 text-xs text-paper">
            Avatar research: {progress.completed} completed, {progress.failed} failed, {progress.total} total modules.
          </div>
        )}
        ...
        {(workspace?.records.length ?? 0) > 0 ? (
          <Panel title="Buyer-role registry and decision models" meta={`${workspace?.records.length ?? 0} records`}>
              ...
              {workspace?.records.map((record) => (
                <div key={record.id} className="bg-ink-200 p-4">
                  <div className="mb-2 flex items-center justify-between gap-2">
                    ...
                    <Tag kind="muted">{readable(record.record_type)}</Tag>
                  </div>
          ...
          <div className="rounded-[10px] border border-dashed border-line bg-ink-200 p-8 text-center">
            <h3 className="text-sm font-medium text-paper">No Avatar OS release yet</h3>
            <p className="mx-auto mt-2 max-w-md text-xs leading-5 text-paper-3">
              Approve Market OS first, then build Avatar OS to map the real buying participants and how they decide—without fictional biographies or invented demographics.
            </p>
          </div>
        )}
