...
import { createIterationCandidate, fetchAnalyticsForClient, fetchIterationCandidates, fetchPerformanceAnalysisRuns, runDeterministicPerformanceAnalysis, updateIterationCandidateStatus, promoteIterationCandidateToOpportunity } from "@/lib/api";
...
import { PlatformExperimentsSection } from "./PlatformExperimentsSection";
...
export function PerformanceIterationPanel({ clientId, executionMonth }: { clientId:string; executionMonth:string }) {
  ...
  async function promote(candidate: ClientIterationCandidate, target: "content_opportunity" | "ad_opportunity") {
    const title = window.prompt(`Title for the new ${target === "content_opportunity" ? "Content" : "Ad"} Opportunity:`, candidate.recommendation.slice(0, 120));
    if (!title?.trim()) return;
    try { setBusy(true); setError(null); await promoteIterationCandidateToOpportunity({ candidateId: candidate.id, target, title: title.trim() }); await load(); }
    catch (value) { setError(errorText(value)); } finally { setBusy(false); }
  }
