...
export function MarketOSPanel({ clientId, clientName }: { clientId: string; clientName: string }) {
  ...
  const [progress, setProgress] = useState<RunMarketOSStepResponse["progress"] | null>(null);
  ...
  const reload = useCallback(async () => {
    setLoading(true);
    try {
      setWorkspace(await fetchIntelligenceWorkspace(clientId, "market_os"));
      setError(null);
    } catch (caught) {
  ...
  async function build() {
    ...
    setProgress(null);
    try {
      const result = await driveMarketOSBuild(clientId, setProgress);
  ...
  async function review(decision: "approved" | "changes_requested") {
    ...
    if (decision === "approved") {
      ...
      const entered = window.prompt("What must change before this Market OS can be approved?");
    ...
    try {
      ...
      setMessage(decision === "approved" ? "Market OS approved and activated." : "Changes requested. The release is back in draft.");
      await reload();
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : String(caught));
    } finally {
  ...
  if (loading && !workspace) {
    return <div className="flex flex-1 items-center justify-center text-xs text-paper-3">Loading Market OS…</div>;
  }
  ...
  const buildLabel = working
    ...
    : latest?.status === "approved" || active ? "Refresh Market OS" : run?.retryable ? "Resume Market OS" : run?.status === "failed" ? "Rebuild Market OS" : run ? "Resume Market OS" : "Build Market OS";
      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-4">
        <div className="flex flex-wrap items-start justify-between gap-4 rounded-[10px] border border-line bg-ink-200 p-4">
          <div className="max-w-2xl">
            ...
            <h2 className="text-base font-medium text-paper">Market OS</h2>
            <p className="mt-1 text-xs leading-5 text-paper-3">
              An evidence-backed model of the client&apos;s commercial environment. Research creates a reviewable draft; only human approval makes it authoritative.
            </p>
          </div>
        ...
        {progress && working && (
          <div className="rounded-[10px] border border-teal/20 bg-teal/5 px-4 py-3 text-xs text-paper">
            Market research: {progress.completed} completed, {progress.failed} failed, {progress.total} total modules.
          </div>
        )}
        ...
        {(workspace?.records.length ?? 0) > 0 ? (
          <Panel title="Market model" meta={`${workspace?.records.length ?? 0} records`}>
          ...
          <div className="rounded-[10px] border border-dashed border-line bg-ink-200 p-8 text-center">
            <h3 className="text-sm font-medium text-paper">No Market OS release yet</h3>
            <p className="mx-auto mt-2 max-w-md text-xs leading-5 text-paper-3">
              Build Market OS to research the market, assemble evidence and findings, and create a versioned draft for review.
            </p>
          </div>
        )}
