// Phase 2A-A — one-button Market OS orchestration.
//
...
async function loadMarketAuthority(sb: ServiceClient, clientId: string): Promise<
  | { ok: true; authority: MarketAuthority }
  | { ok: false; status: number; code: string; message: string; details?: Record<string, unknown> }
> {
  const [clientResult, contextResult] = await Promise.all([
  ...
  if (clientResult.data.stage1_status !== "complete" || approved.length !== 21 || expected.size > 0) {
    return {
      ...
      message: "Market OS requires all 21 approved, non-empty Context Files.",
      details: { stage1_status: clientResult.data.stage1_status, approved: approved.length, missing_file_numbers: [...expected] },
    };
  }
  return { ok: true, authority: { client: clientResult.data, files: approved } };
}
...
async function archiveStaleRun(sb: ServiceClient, clientId: string, runId: string, releaseId: string | null) {
  ...
  await audit(sb, "market_os.stale_run_archived", "client_research_runs", runId, { client_id: clientId, release_id: releaseId });
}

async function prepare(sb: ServiceClient, clientId: string, userId: string) {
  const authorityResult = await loadMarketAuthority(sb, clientId);
  ...
  const { data: openRuns, error: openRunError } = await sb.from("client_research_runs")
    .select("*").eq("client_id", clientId).eq("intelligence_domain", "market_os")
  ...
  for (const run of openRuns ?? []) {
    ...
    return json({
      ok: true,
      mode: "resumed",
      message: "Resuming the existing Market OS build.",
  ...
  const { data: latestRelease, error: versionError } = await sb.from("client_intelligence_releases")
    .select("version").eq("client_id", clientId).eq("intelligence_domain", "market_os")
  ...
  if (runError) {
    if (runError.code === "23505") {
      ...
      if (duplicate) {
        ...
        return json({ ok: true, mode: "resumed", message: "Resuming the existing Market OS build.", research_run_id: duplicate.id, release_id: release?.id, steps: steps ?? [] });
      }
    }
  ...
  const stepRows = MARKET_RESEARCH_MODULES.map((module, index) => ({
    client_id: clientId,
    research_run_id: run.id,
    step_key: module.key,
    step_order: index + 1,
  ...
  await sb.from("client_intelligence_refresh_policies").upsert({
    ...
    policy_reason: "Market assumptions should be reviewed quarterly or when material commercial evidence changes.",
    updated_by: userId,
  }, { onConflict: "client_id,intelligence_domain" });
  await audit(sb, "market_os.prepared", "client_intelligence_releases", release.id, {
    client_id: clientId, research_run_id: run.id, version, authority_hash: authorityHash,
  });
  return json({
    ok: true,
    mode: "prepared",
    message: "Market OS research workflow prepared.",
    research_run_id: run.id,
    release_id: release.id,
    steps: steps ?? [],
  }, 201);
...
async function persistModule(input: {
  ...
  const sourceRows = result.sources.map((source) => ({
    ...
    evidence_text: `Source consulted during the ${step.title} Market OS module.`,
    confidence: "medium",
  ...
  const contextByNumber = new Map(authorityFiles.map((file) => [file.file_number, file]));
  ...
  for (const [recordIndex, record] of result.output.records.entries()) {
    ...
    for (const providerFinding of record.findings) {
      ...
      const hasEvidence = matchedSources.length > 0 || matchedContext.length > 0;
  ...
  if (recordRows.length === 0) throw new Error("Market OS module returned no structured domain records.");
...
function normaliseFinding(finding: MarketModuleFinding, hasEvidence: boolean): MarketModuleFinding {
...
async function runStep(sb: ServiceClient, clientId: string, researchRunId: string) {
  const { data: run, error: runError } = await sb.from("client_research_runs")
    .select("*").eq("id", researchRunId).eq("client_id", clientId).eq("intelligence_domain", "market_os").maybeSingle();
  if (runError || !run) return json({ ok: false, terminal: true, message: runError?.message ?? "Market OS research run not found." }, 404);
  if (run.status === "completed" || run.status === "completed_partial" || run.status === "cancelled") {
    ...
    return json({ ok: true, terminal: true, message: "Market OS research is already terminal.", research_run_id: researchRunId, progress });
  }
  ...
  if (releaseError || !release) return json({ ok: false, terminal: true, message: releaseError?.message ?? "Draft Market OS release not found." }, 409);

  const leaseOwner = `market-os:${crypto.randomUUID()}`;
  ...
  if (!claimed) {
    ...
    return json({
      ...
      message: progress.terminal ? "No recoverable Market OS steps remain." : "Another worker currently owns the next Market OS step.",
  ...
  const authorityChanged = !expectedAuthority || expectedAuthority.some((file) => currentById.get(file.id) !== file.version);
  if (authorityChanged) {
    const message = "Approved Context changed after this Market OS run began. Start a new run from the new authority.";
  ...
  const module = MARKET_RESEARCH_MODULES.find((candidate) => candidate.key === step.step_key);
  ...
  try {
    ...
    await persistModule({
      ...
      authorityFiles: authorityResult.authority.files, result: providerResult,
    });
...
async function finalize(sb: ServiceClient, clientId: string, researchRunId: string) {
  ...
  if (error || !runResult.data || !releaseResult.data) return json({ ok: false, message: error?.message ?? "Market OS run or release not found." }, 404);
  const run = runResult.data;
  const release = releaseResult.data;
  if (release.status === "needs_review" || release.status === "approved") {
    return json({ ok: true, message: "Market OS release is already finalised.", release, run });
  }
  const steps = (stepsResult.data ?? []) as ResearchStepRow[];
  const progress = await stepProgress(sb, researchRunId);
  if (!progress.terminal || progress.completed === 0) {
    return json({ ok: false, message: "Market OS cannot be finalised while recoverable research remains or no module completed.", progress }, 409);
  }
  ...
  if (records.length === 0) return json({ ok: false, message: "Market OS has no structured domain records." }, 409);
  ...
  const { data: updatedRun, error: updateRunError } = await sb.from("client_research_runs").update({
    ...
    failure_message: progress.failed > 0 ? `${progress.failed} Market OS module(s) did not complete.` : null,
  }).eq("id", run.id).select("*").single();
  if (updateRunError) return json({ ok: false, message: updateRunError.message }, 500);
  await audit(sb, "market_os.submitted_for_review", "client_intelligence_releases", release.id, {
    client_id: clientId,
    research_run_id: run.id,
...
async function retryFailedStep(
  ...
  const [runResult, releaseResult, stepResult] = await Promise.all([
    sb.from("client_research_runs").select("*")
      .eq("id", researchRunId).eq("client_id", clientId).eq("intelligence_domain", "market_os").maybeSingle(),
  ...
  if (!runResult.data || !releaseResult.data || !stepResult.data) {
    return json({ ok: false, message: "The failed Market OS module is no longer available to retry." }, 404);
  }
  if (stepResult.data.status !== "failed") {
    return json({ ok: false, message: "Only a failed Market OS module can be retried." }, 409);
  }
  ...
  await audit(sb, "market_os.failed_module_requeued", "client_research_steps", stepId, {
    client_id: clientId,
    research_run_id: researchRunId,
