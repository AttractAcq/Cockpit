...
async function retryFailedStep(
  ...
  const [runResult, releaseResult, stepResult] = await Promise.all([
    sb.from("client_research_runs").select("*")
      .eq("id", researchRunId).eq("client_id", clientId).eq("intelligence_domain", "brand_strategist").maybeSingle(),
  ...
  if (!runResult.data || !releaseResult.data || !stepResult.data) {
    return json({ ok: false, message: "The failed Brand Strategist module is no longer available to retry." }, 404);
  }
  if (stepResult.data.status !== "failed") {
    return json({ ok: false, message: "Only a failed Brand Strategist module can be retried." }, 409);
  }
  ...
  await audit(sb, "brand_strategist.failed_module_requeued", "client_research_steps", stepId, {
    client_id: clientId,
    research_run_id: researchRunId,
