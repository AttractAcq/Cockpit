...
declare v_brief public.client_production_briefs; v_id uuid; v_prompt text;
begin
  ...
  insert into public.client_ai_background_image_generations(client_id,production_brief_id,source_ref,format,frame_index,prompt_text,prompt_created_by)
  values(p_client_id,p_production_brief_id,trim(p_source_ref),p_format,p_frame_index,v_prompt,case when auth.role()='service_role' then 'system' else auth.uid()::text end)
...
declare v_row public.client_ai_background_image_generations; v_prompt text; v_status text;
begin
  if auth.role() <> 'service_role' and (auth.uid() is null or coalesce(public.auth_role(),'') not in ('admin','account_manager','editor')) then raise exception 'AUTH: staff role required'; end if;
...
create or replace function public.generate_ai_background_image(p_generation_id uuid,p_image_size text default null,p_image_quality text default null)
...
begin
  ...
  return query update public.client_ai_background_image_generations
    set prompt_status='generating',image_size=coalesce(nullif(trim(p_image_size),''),'1024x1536'),
      image_quality=coalesce(nullif(trim(p_image_quality),''),'medium'),error_message=null,updated_at=now()
    where id=p_generation_id and prompt_status='approved'
    ...
    returning *;
  if not found then raise exception 'VALIDATION: prompt must be approved and have no active generation'; end if;
end $$;

revoke all on function public.create_ai_background_prompt(uuid,uuid,text,text,integer,text,text),public.update_ai_background_prompt(uuid,text,text,text),public.generate_ai_background_image(uuid,text,text) from public,anon;
...
grant execute on function public.generate_ai_background_image(uuid,text,text) to service_role;
...
comment on function public.generate_ai_background_image(uuid,text,text) is 'Service-role atomic claim only; the Edge Function performs provider and storage work after staff authentication.';
